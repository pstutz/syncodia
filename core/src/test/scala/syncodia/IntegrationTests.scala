...
class IntegrationTests extends FunSuite:
  ...
  test("basic math") {
    ...
    api.executeContinuously(
      ...
      assertEquals(response.content, "The power of three of the square root of 2 is approximately 2.8284271247461907.")
    }
  }
  ...
  test("fahrenheit to celsius conversion") {
    ...
    api.executeContinuously(
      ...
      functions = Seq(ChatFunction(fahrenheitToCelsius, "Converts a temperature from Fahrenheit to Celsius."))
    ).map { response =>
  ...
  test("get location coordinates") {
    ...
    api.executeContinuously(
      ...
      functions = Seq(ChatFunction(getLocationCoordinates, "Returns coordinates for a location."))
    ).map { response =>
  ...
  test("record and retrieve weather information from text") {
    ...
    def recordWeatherInfo(location: Location, temperature: Temperature): Weather = Weather(location, temperature)
